...
const int pwmPIN[]={A1}; // an array to identify the PWM input pins (the array can be any length) 
...
int print_RCpwm(){                             // display the raw RC Channel PWM Inputs
  for (int i = 0; i < RC_inputs; i++){
    //Serial.print(" ch");Serial.print(i+1);
    //Serial.print("  ");
    if(PW[i] < 1000) Serial.print("");
    //Serial.print(map(PW[i],1500,2000,0,100));
    out=map(PW[i],1000,2000,0,255);
  //Serial.print("H");
    return out;
  }
  //Serial.println("");
